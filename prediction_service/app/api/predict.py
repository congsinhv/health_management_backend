...
async def predict(user_input: UserInput, predict_service=Depends(get_predict_service)):
...
async def service_info(predict_service=Depends(get_predict_service)):
    ...
    try:
        model_info = (
            predict_service.model_loader.get_model_info()
            if predict_service.model_loader
            else {"loaded": False}
        )
        return {
