...
def override_device_dependency(mock_schedule_service_device):
    app.dependency_overrides[get_schedule_service] = lambda: mock_schedule_service_device
...
@pytest.mark.asyncio
async def test_register_device_success(client, auth_token, override_device_dependency, mock_schedule_service_device):
    device_data = {
        "fcm_token": "a" * 150,
        "device_type": "android",
        "device_name": "Pixel 6"
...
@pytest.mark.asyncio
async def test_unregister_device_success(client, auth_token, override_device_dependency, mock_schedule_service_device):
    token = "a" * 150
    response = await client.delete(
        f"/api/v1/devices/{token}",
        headers={"Authorization": f"Bearer {auth_token}"}
    )
...
@pytest.mark.asyncio
async def test_unregister_device_not_found(client, auth_token, override_device_dependency, mock_schedule_service_device):
