...
async def test_sse_streaming_full_flow(
    ...
    async with async_client.stream(
        "POST", "/api/v1/qa/ask-stream", json=sample_question_request, headers=headers
    ) as response:
        ...
        assert (
            len(events) >= 3
        )  # At minimum: QUESTION_RECEIVED, ANSWERS_FOUND, STREAM_COMPLETE
...
async def test_sse_invalid_question_data(async_client, sample_user):
    ...
    async with async_client.stream(
        "POST", "/api/v1/qa/ask-stream", json=invalid_data, headers=headers
    ) as response:
