...
@pytest.mark.asyncio
async def test_create_scheduled_notification(db_pool, test_user, test_schedule_plan, mock_connection):
    ...
    plan_id = test_schedule_plan.get("id", 1) if isinstance(test_schedule_plan, dict) else 1
...
async def test_query_pending_notifications(db_pool, mock_connection):
    ...
    pending = await repo.get_pending_in_window(window_start=window_start, window_end=window_end)
