...
def assert_success_response(
    ...
    assert isinstance(response.timestamp, datetime), "Response timestamp should be datetime"
...
def assert_error_response(
    ...
    if expected_message_contains:
        assert expected_message_contains in response.message, f"Message '{response.message}' should contain '{expected_message_contains}'"
...
def assert_exception_matches_schema(
    ...
    assert isinstance(exception, expected_type), f"Expected {expected_type}, got {type(exception)}"
    ...
    assert hasattr(exception, 'error_code'), "Exception should have error_code attribute"
    ...
    assert isinstance(exception.error_code, str), "Exception error_code should be string"

    if expected_message_contains:
        assert expected_message_contains in exception.message, f"Exception message '{exception.message}' should contain '{expected_message_contains}'"
...
def assert_http_status_code(
    ...
    assert actual_code == expected_code, f"Expected HTTP {expected_code}, got {actual_code}"


def create_mock_conversation(
    conversation_id: int = 1,
    user_id: int = 1,
    title: str = "Health Questions"
) -> Dict[str, Any]:
...
def assert_sanitized_error_details(
    ...
    if user_context:
        assert "password" not in sanitized, "Sensitive fields should be removed in user context"
        assert "token" not in sanitized, "Sensitive fields should be removed in user context"
    else:
        if should_redact:
            assert sanitized["password"] == "[REDACTED]", "Sensitive fields should be redacted"
            assert sanitized["token"] == "[REDACTED]", "Sensitive fields should be redacted"

    # Check non-sensitive fields are preserved
    assert "public_field" in sanitized, "Non-sensitive fields should be preserved"
    assert sanitized["public_field"] == "public_value", "Non-sensitive fields should have original values"
...
def assert_pagination_response(
    ...
    assert response.total_pages == (expected_total + expected_page_size - 1) // expected_page_size
...
def assert_datetime_close(
    ...
    diff = abs(dt1 - dt2)
    assert diff.total_seconds() <= tolerance_seconds, f"Datetime difference {diff.total_seconds()}s exceeds tolerance {tolerance_seconds}s"
