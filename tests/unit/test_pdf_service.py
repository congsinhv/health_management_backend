...
@pytest.mark.asyncio
async def test_generate_and_upload_pdf_not_found(pdf_service, mock_connection):
...
@pytest.mark.asyncio
async def test_generate_and_upload_pdf_no_gcs(pdf_service_no_gcs, mock_connection):
    ...
    with patch.object(
        ...
        url = await pdf_service_no_gcs.generate_and_upload_pdf("test-prediction-123")

        # Should return None when GCS is not available
        assert url is None
...
async def test_generate_pdf_bytes_success(pdf_service):
    ...
    with patch.object(pdf_service, "_html_to_pdf") as mock_pdf:
...
async def test_prepare_template_context_invalid_json_strings(pdf_service, caplog):
    ...
    assert context["prediction_id"] == "test-prediction-123"
    # When JSON parsing fails, error should be logged and defaults should be used
    assert any(
        "Failed to parse user_input as JSON" in record.message
        for record in caplog.records
    )
    # Should use default values when JSON parsing fails
    assert context["user_input"]["name"] == "User"  # Default fallback
    assert context["user_input"]["gender"] == "Nữ"  # Default fallback
    assert context["prediction"]["bmi"] == 22.5
...
async def test_prepare_template_context_null_values(pdf_service):
    ...
    assert context["prediction_id"] == "test-prediction-123"
    # Should handle None values gracefully by using default values
    assert context["user_input"]["name"] == "User"  # Default fallback
    assert (
        context["user_input"]["gender"] == "Nữ"
    )  # Default fallback since None.get("gender") returns None
    assert context["prediction"]["bmi"] == 22.5
