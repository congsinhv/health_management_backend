...
class TestObesityPredictorComplete:
    ...
    def test_ensure_models_downloaded_from_gcs(self, mock_model_files):
        ...
        with patch("os.path.exists", return_value=False):
            with patch("app.services.predict_service.settings") as mock_settings:
                ...
                with patch(
                    "app.services.predict_service.GCSDownloader"
                    ...
                    assert (
                        calls[0][0][0] == "models_obesity/obesity_classifier_final.pkl"
                    )
        ...
        with patch("os.path.exists", return_value=False):
            with patch("app.services.predict_service.settings") as mock_settings:
                ...
                with patch(
                    ...
                    with patch("pathlib.Path.mkdir"):
                        with pytest.raises(
                            RuntimeError, match="Failed to download model file from GCS"
                        ):
                            predictor._ensure_models_downloaded(
                                mock_model_files["model_path"],
                                mock_model_files["encoder_path"],
    ...
    @pytest.mark.skip(
        reason="format_bmi method not implemented in ObesityPredictorComplete"
    )
    ...
    @pytest.mark.skip(
        reason="build_prompts method not implemented in ObesityPredictorComplete"
    )
    ...
    @pytest.mark.skip(
        reason="get_ai_suggestion method not implemented in ObesityPredictorComplete"
    )
